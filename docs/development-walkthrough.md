...
## 15. Demo Data And Run Documentation

The final documentation slice makes the project easier to run during review or presentation:

- `database/seed.sql` now creates or updates a demo user with a real PBKDF2 password hash generated by the Infrastructure `PasswordHasher`.
- Demo credentials are documented as `demo@ballastlane.com` / `Password123!`.
- `README.md` explains how to reset LocalDB, start the API, start Angular, run backend tests, run frontend tests, and build the frontend.
- The README also includes the required GenAI usage section and explains how AI output was validated.

At this point, the project has the required backend, frontend, database scripts, tests, and documentation for the technical exercise.
